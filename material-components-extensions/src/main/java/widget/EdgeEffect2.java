...
public class EdgeEffect2 extends EdgeEffect {
    ...
    private static final String TAG = "EdgeEffect2";
     ...
     */
    public @EdgeEffectType int mEdgeEffectType = TYPE_GLOW;
     ...
     * Construct a new EdgeEffect with a theme appropriate for the provided context.
     *
     * @param context Context used to provide theming and resource information for the EdgeEffect2
     * @param attrs   The attributes of the XML tag that is inflating the view
     */
    public EdgeEffect2(@NonNull Context context, @Nullable AttributeSet attrs) {
        super(context);
    }
    ...
    public void onAbsorb(int velocity) {
        ...
        if (edgeEffectBehavior == TYPE_STRETCH) {
            ...
            mGlowScaleYFinish = Math.min(0.025f + (velocity * (velocity / 100f) * 0.00015f) / 2, 1.f);
    ...
    public boolean draw(Canvas canvas) {
        ...
        if (edgeEffectBehavior == TYPE_GLOW) {
            ...
            if (edgeEffectBehavior == TYPE_STRETCH && canvas instanceof RecordingCanvas) {
                if (mState == STATE_RECEDE) {
                    updateSpring();
                }
                if (mDistance != 0f) {
                    RecordingCanvas recordingCanvas;
                    recordingCanvas = (RecordingCanvas) canvas;
                    if (mTmpMatrix == null) {
                    ...
                    @SuppressLint("PrivateApi") RenderNode renderNode = null;
                    try {
                        renderNode = (RenderNode) recordingCanvas.getClass().getDeclaredField("mNode").get(recordingCanvas);
                    ...
                    if (right > left && bottom > top && mWidth > 0 && mHeight > 0 && hasValidVectors) {
                        try {
                            renderNode.getClass().getDeclaredMethod("stretch", new Class[]{float.class, float.class, float.class, float.class}).invoke(renderNode, vecX, vecY, mWidth, mHeight);
    ...
    private void updateSpring() {
        ...
        if (Math.abs(mVelocity) <= LINEAR_VELOCITY_TAKE_OVER && Math.abs(mDistance * mHeight) < LINEAR_DISTANCE_TAKE_OVER && Math.signum(mVelocity) == -Math.signum(mDistance)) {
        ...
        double sinCoeff = (1 / mDampedFreq) * (DAMPING_RATIO * NATURAL_FREQUENCY * mDistance * mHeight + mVelocity);
        double distance = Math.pow(Math.E, -DAMPING_RATIO * NATURAL_FREQUENCY * deltaT) * (cosCoeff * Math.cos(mDampedFreq * deltaT) + sinCoeff * Math.sin(mDampedFreq * deltaT));
        double velocity = distance * (-NATURAL_FREQUENCY) * DAMPING_RATIO + Math.pow(Math.E, -DAMPING_RATIO * NATURAL_FREQUENCY * deltaT) * (-mDampedFreq * cosCoeff * Math.sin(mDampedFreq * deltaT) + mDampedFreq * sinCoeff * Math.cos(mDampedFreq * deltaT));
    ...
    private boolean isAtEquilibrium() {
        ...
        return displacement < 0 || (Math.abs(velocity) < VELOCITY_THRESHOLD && displacement < VALUE_THRESHOLD);
    }
