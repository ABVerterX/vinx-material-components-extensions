...
public class FastScrollWebView extends WebView implements ViewHelperProvider {
    ...
    private class ViewHelper extends SimpleViewHelper {
        ...
        protected void superDraw(@NonNull Canvas canvas) {
            vinx.material.fastscroll.FastScrollWebView.super.draw(canvas);
        }

        @Override
        protected void superOnScrollChanged(int left, int top, int oldLeft, int oldTop) {
            vinx.material.fastscroll.FastScrollWebView.super.onScrollChanged(left, top, oldLeft, oldTop);
        }

        @Override
        protected boolean superOnInterceptTouchEvent(@NonNull MotionEvent event) {
            return vinx.material.fastscroll.FastScrollWebView.super.onInterceptTouchEvent(event);
        }

        @Override
        protected boolean superOnTouchEvent(@NonNull MotionEvent event) {
            return vinx.material.fastscroll.FastScrollWebView.super.onTouchEvent(event);
        }

        @Override
        protected int computeVerticalScrollRange() {
            return vinx.material.fastscroll.FastScrollWebView.this.computeVerticalScrollRange();
        }

        @Override
        protected int computeVerticalScrollOffset() {
            return vinx.material.fastscroll.FastScrollWebView.this.computeVerticalScrollOffset();
        }

        @Override
        protected int getScrollX() {
            return vinx.material.fastscroll.FastScrollWebView.this.getScrollX();
        }

        @Override
        protected void scrollTo(int x, int y) {
            vinx.material.fastscroll.FastScrollWebView.this.scrollTo(x, y);
        }
    }
}
